...
public class CourtController {
    ...
    public ResponseEntity<Object> createCourt(@RequestBody CreateCourtCommand command, @PathVariable(name = "userId") Long userId) {
        try {
            return ResponseEntity.ok(courtService.createCourt(command, userId)); // 200
        } catch (Exception e) {
    ...
    public ResponseEntity<Object> getCourtByUser(@PathVariable(name = "userId") Long userId) {
        try {
            return ResponseEntity.ok(courtService.getCourtsByUser(userId));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(null);
        }
    }
    ...
    public ResponseEntity<Object> getCourtReservations(@PathVariable(name = "courtId") Long courtId) {
        try {
            return ResponseEntity.ok(courtService.getCourtReservations(courtId));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(null);
        }
    }
